...
public class VRPTWServiceImpl implements VRPTWService {
    ...
    private VehicleRoutingProblem.Builder createProblem(List<Destination> destinations, LocalTime WAITING_TIME) {
        ...
        for (Destination destination : destinations.subList(1, destinations.size())) {
            ...
            // addSizeDimension ??
            com.graphhopper.jsprit.core.problem.job.Service service = com.graphhopper.jsprit.core.problem.job.Service.Builder.newInstance(destination.getId())
    ...
    private VehicleImpl defineVehicle(Destination startingDestination, LocalTime earliestStart, LocalTime latestArrival,
                                     int MAX_DESTINATIONS_PER_DAY) {
    ...
    private VehicleRoutingTransportCostsMatrix createMatrix(List<Destination> destinations, double[][] distances, double[][] durations) {
